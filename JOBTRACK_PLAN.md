...
- 当前全量测试为 397 passed，覆盖率 92.49%，PostgreSQL 和 Redis 集成测试全部通过。
...
- Review 优化 3 已完成：复合外键在数据库层保证 Job、Application、Interview 与父资源的 owner 一致。
- Review 优化 4 已完成：生产环境强制使用共享 Redis 限流，并对全局和业务写限流执行 fail-closed。
...
- `ARCHITECTURE.md` 已包含 ER 图和认证请求流程；八份 ADR 已记录关键架构取舍。
...
- 最终验证：397 passed，覆盖率 92.49%，Ruff 和发行包构建通过。
...
- 后续 Review 已用复合外键关闭绕过 Service 时可能产生的跨 owner 父子关联，并验证用户删除仍可完整级联。
- 后续 Review 已消除生产多实例使用进程内限流或 Redis 故障时放行写请求的配置风险，并增加启动期校验。
